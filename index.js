...
function buildPromptOrigin(cmd, topic) {
  switch (cmd) {
    case '/hook':
      return `你是一个资深小红书种草内容策划师，专门为咖啡馆、餐饮、生活方式类账号撰写爆款"钩子型"开头内容，用于发布图文笔记。

请基于以下品牌/内容背景，生成 4 条适合用于小红书图文笔记开头的"钩子内容"文案：

要求：
- 每条钩子需具有强吸引力和可读性，风格应包含反差、共鸣、情绪、好奇、利益诱导等维度
- 不要标注"风格名称"（如 emotional、contrast 等）
- 使用"人话"说法，避免模板语气和直译句
- 偶尔可使用 emoji，但应自然融入句子中，不能每句都统一格式
- 每条不超过 2 行，控制在 15～25字以内（或 1 句话）
